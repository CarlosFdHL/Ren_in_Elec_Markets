...
class Step6_model:
    # Step6_model is a class that represents the optimization model. It receives an instance of the InputData class to build the optimization model and solve it.

    def __init__(self, input_data: InputData, reserve_results):
    ...
    def build_constraints(self):
            ...
            for t_index, t in enumerate(self.data.timeSpan):
                if not self.data.wind[g]:
                    constraint = self.model.addConstr(
                        ...
                        GRB.LESS_EQUAL,
                        self.data.Pmax[g] - self.reserve_results.reserve_up[g, t],
        ...
        self.constraints.production_lower_limit = {
            (g, t): self.model.addConstr(self.variables.production[g, t], 
                                         GRB.GREATER_EQUAL, 
                                         self.data.Pmin[g] + self.reserve_results.reserve_down[g, t],
    ...
    def save_results(self):
        ...
        self.results.total_profit = self.results.profit_data
        for t in self.data.timeSpan:
            for g in self.data.generators:
                self.results.total_profit.at[t, g] = self.results.profit_data.at[t, g] + self.reserve_results.profit_reserve.at[t, g]
    ...
    def print_results(self):
        ...
        print("\n3.-Profit for each producer on Day-Ahead Market")
        print(self.results.profit_data)

        print("\nTotal profit for each producer (DA + Reserve)")
        print(self.results.total_profit)
